...
const std::vector <coord<int> > c_TicTacToeBoard::get_legal_moves(){
...
int c_TicTacToeBoard::check_winner(){
    // horizontal
    for (int x = 0; x < h; x++){
...
void c_TicTacToeBoard::execute_move(const coord<int> &pos, const int &color){
    board[pos.x][pos.y] = color;
    step_cnt += 1;
}

bool c_TicTacToeBoard::has_legal_moves(){
    return step_cnt < h*w;
}

void c_TicTacToeBoard::reset(){
    step_cnt = 0;
    for (int x = 0; x < h; x++){
        for (int y = 0; y < w; y++){
            board[x][y] = EMPTY;
...
const std::vector <int> & c_TicTacToeBoard::get_board() const{
